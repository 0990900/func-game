...
- **조커 한 장은 한 컨테이너의 한 연산을 맡는다.** 그 자리는 진짜 카드와 똑같이 동작해서, `Maybe.ap`으로
  배정된 조커는 Maybe Apply와 Maybe Monad를 함께 지원한다. 대신 동시에 다른 연산이 될 수는 없다.
...
- 조합마다 조커 풀을 새로 만들지 않는다. 그렇게 하면 조커 한 장이 네 컨테이너의 Monad를 동시에 세운다.
