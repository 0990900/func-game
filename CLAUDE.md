...
- **`.ts`를 직접 실행하는 스크립트에는 `--experimental-strip-types`를 붙인다.** Node 23.6부터는 없어도
  돌아가지만 22.6~23.5에서는 이게 없으면 시작조차 못 한다. 내 기기에서 된다고 확인을 건너뛰면 다른
  기기에서 `npm start`가 실패한다 — 실제로 그렇게 나갔다. `engines`에 최소 버전을 적어둔다.
...
| Utility Belt | Utility 3장 이상 | 6 | 51% |
...
지금은 46~69%다. 유틸리티 곡선을 올리자 Utility Belt가 2장 기준으로는 81%까지 뛰어 3장으로 올렸다 — **한쪽 숫자를 바꾸면 목표 난이도가 따라 움직인다.** 게임 구조를 바꾸면 이 숫자들이 다시 흔들리므로 그때마다 재서 맞춘다.
...
Utility 카드는 덱에 12장, 전부 다른 것으로 한 장씩 들어 있다. 조합에 쓰이지 않고 **몇 장을 모았는지로만**
점수가 된다(한 장씩뿐이라 "종류 수"와 "장수"가 같은 말이다).

| 장수 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 점수 | 2 | 5 | 9 | 14 | 20 | 27 | 35 | 44 |
| 한 장당 | +2 | +3 | +4 | +5 | +6 | +7 | +8 | +9 |

**k번째 카드는 k+1점이다.** 모을수록 커지고, 표 바깥도 같은 규칙으로 이어진다.

곡선은 두 번 손봤고 두 번 다 같은 증상에서 출발했다 — **시장이 유틸리티로 막히는 것.** 처음엔 1종이
0점이라 아무도 첫 장을 집지 않았다. 전 구간 +1로 그건 없앴지만, 그 뒤 조합이 13개로 늘고 빙고가 붙어
컨테이너 카드 한 장이 4~5점이 되는 동안 유틸리티는 첫 장 1점에 머물렀다. 시장 네 자리 중 2.68장이
유틸리티로 막혀 아무도 교환하지 않았고, 네 명 중 한 명은 한 장도 없이 끝냈다. 지금 곡선에서 시장에 남은
유틸리티는 0.67장이고 세 장 이상 모으는 사람이 18%에서 27%로 늘었다.

**시장에 남은 유틸리티 장수가 이 곡선의 감시 지표다.** 1장을 넘기면 다시 막히고 있다는 뜻이다.
